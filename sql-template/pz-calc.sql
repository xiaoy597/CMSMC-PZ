...
CREATE VOLATILE MULTISET TABLE VT_OAP_SEC_ACCT AS(
	SELECT A.OAP_ACCT_NBR
		,A.SEC_ACCT
		,A.SEC_ACCT_NAME
	FROM NsoVIEW.CSDC_INTG_SEC_ACCT A, $PARAM{'CMSSDB'}.PZ_LIST_HIS B
	WHERE B.LD_NBR = '$PARAM{'ld_nbr'}'
...
CREATE VOLATILE MULTISET TABLE VT_TRAN_A AS(
	SELECT	
		...
		WHERE  a1.shdr_acct=a2.SEC_ACCT
			  AND	a1.trad_date between CAST('$PARAM{'s_date'}' AS DATE FORMAT 'YYYYMMDD') 
...
FROM (                        	
		...
		,(
			...
			WHERE IF_TRADDAY =1
        		AND CALENDAR_DATE between CAST('$PARAM{'s_date'}' AS DATE FORMAT 'YYYYMMDD')
									AND  CAST('$PARAM{'e_date'}' AS DATE FORMAT 'YYYYMMDD')
        	) B 
GROUP BY 1)
...
from $PARAM{'CMSSDB'}.PZ_KPI_YZ
where e_date = CAST('$PARAM{'max_dt'}' AS DATE FORMAT 'YYYYMMDD'))
...
INSERT INTO $PARAM{'CMSSDB'}.PZ_OAP_KPI_SCR
(
	OAP_ACCT_NBR
	,SEC_ACCT_NAME
	,LD_NBR
